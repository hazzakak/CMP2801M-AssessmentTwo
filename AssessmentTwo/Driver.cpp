...
vector<std::string> splitString(const char in[])
{
	...
	// It does this by creating an iss from the character array, 
...
int main()
{
	...
	while (userCommand.compare("exit") != 0)
	{
		...
		// split the character array into a vector
		...
		if (command.compare("addR") == 0) {
			...
			// or else print an error message and exit the loop.
			if (parameters.size() != 5) {
				cout << "4 parameters are required: 'addR <x> <y> <height> <width>'" << endl;
				goto wrongCmd;
			}

			// convert the string parameters to ints and use them to create a new rectangle object.
			...
			// calculate the area, perimeter, and points of the rectangle.
			r->calculateArea();
			...
			// calculate the area, perimeter, and points of the square.
			s->calculateArea();
			...
			// calculate the area, perimeter, and points of the circle.
			c->calculateArea();
			...
			// check if the shape index is valid.
			// else, print an error message and exit the loop.
			if (shapes.size() < shapeIndex || shapeIndex < 1) {
